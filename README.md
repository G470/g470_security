...
Small utility plugin that protects the REST users endpoint. By default it blocks `/wp/v2/users` for everyone except logged-in users who have `list_users` (or another capability you choose in settings).
...
- Options stored in a single array option `rup_options` with safe defaults and sanitization.
...
## Customization
- Change defaults programmatically: update the `rup_options` option with `['rup_enabled' => true/false, 'rup_capability' => 'your_cap']`.
